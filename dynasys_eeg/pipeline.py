...
class DynaSysPipeline:
        ...
        if len(Z_primary_valid) >= 4:
            ...
            if len(np.unique(y_all)) >= 2:
                ...
                if save_results:
                    ...
                    # ⭐ ADD SECONDARY DATA HERE
                    Z_secondary_valid=Z_secondary_valid if self._secondary_subjects else None,
                    labels_secondary_valid=labels_secondary_valid if self._secondary_subjects else None,
    ...
    def _save_visualizations(
        ...
        # secondary
        Z_secondary_valid=None,
        labels_secondary_valid=None,
    ):
        """Save all visualizations (Primary + Secondary + Cross-Dataset)."""
        ...
        if cm is not None:
            plot_confusion_matrix(
                cm,
                class_names,
                title="Confusion Matrix (LOSO - Primary)",
        ...
        if clf_results:
            plot_comparison_results(
                clf_results,
                title="Method Comparison (Primary Dataset)",
        ...
        sec_results = self.results.get("loso_secondary", None)

        if Z_secondary_valid is not None and labels_secondary_valid is not None:

            Z_sec_all = np.concatenate(Z_secondary_valid, axis=0)

            unique_labels = sorted(set(labels_secondary_valid))
            sec_label_map = {lbl: i for i, lbl in enumerate(unique_labels)}
            sec_inv_map = {i: lbl for lbl, i in sec_label_map.items()}

            y_sec_all = np.concatenate([
                np.full(len(Z), sec_label_map[lbl])
                for Z, lbl in zip(Z_secondary_valid, labels_secondary_valid)
            ])
            ...
            plot_descriptors_distribution(
                ...
                y_sec_all,
                sec_inv_map,
                save_path=str(results_dir / "descriptor_distribution_secondary.png")
            )

            # Secondary LOSO confusion matrix
            if sec_results and sec_results.get("confusion_matrix") is not None:
                cm_sec = sec_results["confusion_matrix"]

                class_names_sec = [sec_inv_map[i] for i in sorted(sec_inv_map.keys())]

                plot_confusion_matrix(
                    cm_sec,
                    class_names_sec,
                    title="Confusion Matrix (LOSO - Secondary)",
                    save_path=str(results_dir / "confusion_matrix_loso_secondary.png")
                )

            # Secondary LOSO summary plot
            if sec_results:
                agg = sec_results.get("aggregate", {})
                ...
                plt.title("Secondary LOSO Performance")
                plt.ylim(0, 1)
                plt.tight_layout()
                plt.savefig(str(results_dir / "secondary_loso_summary.png"))
                plt.close()

        # CROSS-DATASET VISUALIZATION

        cross = self.results.get("cross_dataset", {}).get("Resting→Olfactory", None)
        ...
        if cross:
            ...
            plt.title("Cross-Dataset Performance (Resting → Olfactory)")
            plt.ylim(0, 1)
            plt.tight_layout()
            plt.savefig(str(results_dir / "cross_dataset_performance.png"))
            plt.close()
